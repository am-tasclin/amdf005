...
var initDataModel = function () {
	...
	ctrl.save_model_i18n = function (el) {
		...
		if (el.i18n_id) {
			var so = {
				i18n: el.i18n, i18n_id: el.i18n_id,
				dataAfterSave: function (response) {
			...
			var so = {
				parent: i18n_parent, reference: el.doc_id, i18n: el.i18n,
				dataAfterSave: function (response) {
					console.log(el, response.data, so)
	...
	ctrl.content_menu.pasteElementReference2 = function (el) {
		console.log(el, ctrl.content_menu.copyObject)
		var so = {
			reference2: ctrl.content_menu.copyObject.doc_id,
			doc_id: el.doc_id,
	...
	ctrl.content_menu.pasteElementReference1 = function (el) {
		console.log(el, ctrl.content_menu.copyObject)
		var so = {
			reference: ctrl.content_menu.copyObject.doc_id,
			doc_id: el.doc_id,
...
var read_to_folder = function (d, d_start_id, doc) {
	if (true || !ctrl.eMap[d_start_id]) {//TODO: пропадає іконка закрити робочу праву панель
		...
		readSql({
			sql: sql, afterRead: function (response) {
...
function read_element_descendant(doc_id, fn) {
	...
	if (!o) {
		...
		read_element_children(o.doc_id, {
			forEachOne: function (v, response) {
				if (v.cnt_child > 0) {
